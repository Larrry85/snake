...
func (g *Game) Update() error {
	...
	g.snake.Move()
//////////////////////// These lines are for moving the snake with keypad
...
/////////////////// Keypad moving lines
	...
	for _, part := range g.snake.Body[1:] {
		if head.X == part.X && head.Y == part.Y {
			g.gameOver = true // if snake hits itself, game over
			g.speed = 10
		}
	}
...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	for _, p := range g.snake.Body {
		vector.DrawFilledRect(screen, float32(p.X*tileSize), float32(p.Y*tileSize), float32(tileSize), float32(tileSize), color.RGBA{0, 255, 0, 255}, true)
	}
